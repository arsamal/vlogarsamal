...
import { DataNegeriView } from './components/DataDesaView';
import { SuratMakerView } from './components/SuratMakerView'; // Import new view
import { Dashboard } from './components/Dashboard';
import { LoginView } from './components/LoginView';
import { ChangePasswordModal } from './components/ChangePasswordModal';
import { MENU_ITEMS, BOOK_CATEGORIES, PLANNING_ICON, DATA_NEGERI_FIELDS, DEFAULT_PASSWORD, CUSTOM_PASSWORD_STORAGE_KEY } from './constants';
import { AllDataBooks, BookDefinition, GenericEntry, FieldDefinition, DataNegeriEntry, DATA_NEGERI_KEY, USAGE_GUIDE_KEY, PEMBUAT_SURAT_KEY } from './types';
import { 
  initializeDatabase, 
  loadAllDataFromDb, 
  addEntry, 
...
const AUTH_STORAGE_KEY = 'digitalNegeriAuth';
...
const App: React.FC = () => {
  ...
  const [dataNegeri, setDataNegeri] = useState<DataNegeriEntry | null>(null);
  const [isLoadingDataNegeri, setIsLoadingDataNegeri] = useState(false);
  const [isAccessBlocked, setIsAccessBlocked] = useState<boolean>(false); 

  const [selectedBookKey, setSelectedBookKey] = useState<string | null>(null);
  const [currentView, setCurrentView] = useState<'dashboard' | 'book' | 'planning' | 'data_negeri' | 'usage_guide' | 'surat_maker'>('dashboard');
    ...
    async function initDbAndLoadData() {
      if (isAuthenticated && !db) {
        try {
          ...
          const negeriData = await getDataNegeri(database);
          setDataNegeri(negeriData);

          if (negeriData?.installation_id) {
            const localToken = localStorage.getItem(LOCAL_INSTALLATION_TOKEN_KEY);
            if (localToken !== negeriData.installation_id) {
  ...
  useEffect(() => {
    ...
    } else if (cleanHash === DATA_NEGERI_KEY) {
      setCurrentView('data_negeri');
      setSelectedBookKey(DATA_NEGERI_KEY);
      if (pathname !== '/' || search !== '' || routeHash !== `#/${DATA_NEGERI_KEY}`) {
         navigate(`/#/${DATA_NEGERI_KEY}`, { replace: true });
      }
  ...
  const handleSelectNavigation = useCallback((viewType: 'book' | 'planning' | 'dashboard' | 'data_negeri' | 'usage_guide' | 'surat_maker', key?: string) => {
    if (!isAuthenticated || isAccessBlocked) return; 
    setIsMobileSidebarOpen(false); 

    if (viewType === 'planning') {
      navigate('/planning');
    } else if (viewType === 'data_negeri' && key === DATA_NEGERI_KEY) {
      navigate(`/#/${DATA_NEGERI_KEY}`);
  ...
  const handleDeleteDbEntryApp = async (bookKey: string, id: number) => {
    ...
    try {
      ...
      const updatedBookData = await getAllEntries(db, bookKey);
      setAllData(prevData => ({ ...prevData, [bookKey]: updatedBookData }));
      if (bookKey === 'buku_rpjmnegeri') {
        const rkpToDelete = (allData.buku_rkp || []).filter(rkp => rkp.id_rpjmnegeri_induk === id);
        for (const rkp of rkpToDelete) await handleDeleteDbEntryApp('buku_rkp', rkp.id);
      } else if (bookKey === 'buku_rkp') {
        const apbdesToDelete = (allData.buku_apbnegeri || []).filter(apbdes => apbdes.id_rkp_induk === id);
        for (const apbdes of apbdesToDelete) await handleDeleteDbEntryApp('buku_apbnegeri', apbdes.id);
      }
    } catch (error) { console.error("Gagal menghapus entri via DB:", error); }
  };

  const handleSaveDataNegeri = async (updatedData: Omit<DataNegeriEntry, 'id'>) => {
    if (!db || isAccessBlocked) return;
    try {
      await saveDataNegeri(db, updatedData); 
      const reloadedDataNegeri = await getDataNegeri(db);
      setDataNegeri(reloadedDataNegeri);
      
      if (reloadedDataNegeri?.installation_id) {
        const localToken = localStorage.getItem(LOCAL_INSTALLATION_TOKEN_KEY);
        if (localToken !== reloadedDataNegeri.installation_id) {
           setIsAccessBlocked(true); 
           console.warn("Access blocked post-save: Mismatch after saving Data Negeri.");
        } else {
           setIsAccessBlocked(false); 
        }
      }
      alert('Data Negeri berhasil disimpan.');
    } catch (error) {
      console.error("Gagal menyimpan Data Negeri:", error);
      alert('Gagal menyimpan Data Negeri. Lihat konsol untuk detail.');
    }
  };
  ...
  const handleBackupDatabase = () => {
    ...
    try {
      ...
      link.download = `digital_negeri_backup_${dateString}.db`;
  ...
  const handleRestoreDatabase = async (file: File) => {
    ...
    reader.onload = async (event) => {
      try {
        ...
        const dataNegeriTableFields = [...DATA_NEGERI_FIELDS];
        if (!dataNegeriTableFields.find(f => f.name === 'installation_id')) {
            dataNegeriTableFields.push({ name: 'installation_id', label: 'Installation ID', type: 'text' });
        }
        const dataNegeriColumns = dataNegeriTableFields.map(f => `"${f.name}" ${f.type === 'number' ? 'REAL' : 'TEXT'}`).join(', ');
        newDb.run(`CREATE TABLE IF NOT EXISTS "${DATA_NEGERI_KEY}" (id INTEGER PRIMARY KEY, ${dataNegeriColumns});`);

        MENU_ITEMS.forEach(book => {
            if (book.key !== DATA_NEGERI_KEY && book.fields.length > 0) {
        ...
        setAllData(dataFromDb);
        const negeriData = await getDataNegeri(newDb);
        setDataNegeri(negeriData);
  ...
  const selectedBookDefinition = (currentView === 'book' || currentView === 'data_negeri' || currentView === 'usage_guide' || currentView === 'surat_maker') && selectedBookKey
    ...
    switch (currentView) {
      ...
      case 'data_negeri':
        return (
          <DataNegeriView
            dataNegeri={dataNegeri}
            onSave={handleSaveDataNegeri}
            isLoading={isLoadingDataNegeri}
            fields={DATA_NEGERI_FIELDS}
          />
        );
      case 'usage_guide':
        return (
            <div className="bg-white p-6 sm:p-8 rounded-xl shadow-lg">
                ...
                <div className="prose prose-sm sm:prose-base max-w-none text-gray-700">
                    <p className="lead">Selamat datang di Aplikasi Administrasi Digital Negeri. Aplikasi ini dirancang untuk membantu Anda mengelola berbagai data administrasi negeri secara digital dan efisien.</p>
                    ...
                    <ul className="list-disc pl-5 space-y-1">
                        ...
                        <li><strong>Dashboard:</strong> Halaman awal yang menampilkan ringkasan data penting dan statistik negeri.</li>
                        <li><strong>Perencanaan Negeri:</strong> Fitur terpadu untuk mengelola dokumen RPJMNegeri, RKP Negeri, dan APBNegeri secara berjenjang.</li>
                        <li><strong>Data Umum Negeri:</strong> (Terdapat di bawah kategori "Administrasi Umum") Digunakan untuk mengisi dan mengelola informasi dasar mengenai negeri Anda yang akan digunakan pada kop surat dan laporan. Menyimpan data di sini untuk pertama kali akan mengaktifkan lisensi aplikasi untuk instalasi ini.</li>
                        <li><strong>Petunjuk Penggunaan:</strong> Halaman ini yang sedang Anda baca.</li>
                         <li><strong>Pembuat Surat:</strong> (Di menu "Pembuat Surat") Untuk membuat berbagai jenis surat resmi negeri seperti Surat Undangan, Surat Pemberitahuan, dan Surat Keterangan.</li>
                        <li>Setiap <strong>buku administrasi</strong> (misalnya, Buku Peraturan Negeri, Buku Induk Penduduk) memiliki halaman tersendiri untuk melihat, menambah, mengedit, dan menghapus data.</li>
                    </ul>

                    <h3 className="mt-6 mb-2 text-xl font-semibold text-gray-700">Pengelolaan Data</h3>
                     <ul className="list-disc pl-5 space-y-1">
                        <li><strong>Tambah Data:</strong> Klik tombol "Tambah Entri Baru" (atau sejenisnya, misal "Tambah RPJMNegeri") yang tersedia di setiap halaman buku atau bagian.</li>
                        <li><strong>Edit Data:</strong> Pada tabel data, klik tombol "Edit" pada baris data yang ingin diubah.</li>
                        <li><strong>Hapus Data:</strong> Klik tombol "Hapus" pada baris data. Akan ada konfirmasi sebelum data dihapus permanen. Perhatian: Menghapus data RPJMNegeri juga akan menghapus data RKP Negeri dan APBNegeri terkait, begitu pula menghapus RKP Negeri akan menghapus APBNegeri terkait.</li>
                    ...
                    <h3 className="mt-6 mb-2 text-xl font-semibold text-gray-700">Cetak dan Ekspor Data</h3>
                     <ul className="list-disc pl-5 space-y-1">
                        <li>Setiap halaman buku (dan bagian pada Perencanaan Negeri) menyediakan tombol "Cetak" untuk mencetak tabel data.</li>
                        ...
                        <li><strong>Kop Surat Otomatis:</strong> Pastikan Anda telah mengisi "Data Umum Negeri" dengan lengkap. Informasi seperti nama negeri, kabupaten, kecamatan, alamat, dan logo (jika URL gambar logo diisi) akan digunakan secara otomatis pada kop surat saat mencetak dokumen.</li>
                    </ul>
                     ...
                     <ul className="list-disc pl-5 space-y-1">
                        ...
                        <li><strong>PENTING (Kebijakan Satu Instalasi):</strong>
                            <ul className="list-disc pl-6 mt-1">
                                <li>Aplikasi ini dirancang untuk penggunaan pada <strong>satu instalasi (satu komputer/profil browser)</strong> per dataset. Aktivasi pertama terjadi saat Anda menyimpan "Data Umum Negeri".</li>
                    ...
                    <p className="mt-8 text-center text-gray-600">Terima kasih telah menggunakan Aplikasi Administrasi Digital Negeri!</p>
                </div>
            </div>
        );
      case 'surat_maker':
        return (
            <SuratMakerView dataNegeri={dataNegeri} allData={allData} />
        );
      case 'book':
        if (selectedBookDefinition && selectedBookKey && 
            selectedBookKey !== DATA_NEGERI_KEY && 
          ...
          <div className="flex items-center justify-between">
            ...
            <h1 className="text-lg font-semibold text-sky-600">
              { selectedBookDefinition?.label || 
                (currentView === 'planning' ? 'Perencanaan Negeri' : 
                (currentView === 'surat_maker' ? 'Pembuat Surat' : 
                (currentView === 'dashboard' ? 'Dashboard' : 'Digital Negeri'))) 
              }
            </h1>
